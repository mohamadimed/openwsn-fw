...
void ieee154e_init(void) {
    ...
    */
    ...
    radio_setConfig (cellRadioSettingMap[CELLRADIOSETTING_2]); 
...
{   
    ...
    //40ms slot for FSK
    slot_154e_vars [SLOT_40ms_FSK_SUBGHZ].TsTxOffset             =  ( 5500  /PORT_US_PER_TICK );   //  measured: 131 ticks
    slot_154e_vars [SLOT_40ms_FSK_SUBGHZ].TsLongGT               =  ( 2000  /PORT_US_PER_TICK );   //  measured: 66 ticks
    slot_154e_vars [SLOT_40ms_FSK_SUBGHZ].TsTxAckDelay           =  ( 4200  /PORT_US_PER_TICK );   //  measured: 121 ticks
...
void isr_ieee154e_newSlot(opentimers_id_t id) {
    ...
    if (ieee154e_vars.isSync==FALSE) {
        if (idmanager_getIsDAGroot()==TRUE) {
            ...
            activity_synchronize_newSlot();//debugpins_frame_toggle();
        }
    } else {
#ifdef ADAPTIVE_SYNC
...
void ieee154e_endOfFrame(PORT_TIMER_WIDTH capturedTime) {
    ...
    } else {           // ieee154e_vars.dataToSend->creator = 0x24
        switch (ieee154e_vars.state) { //ieee154e_vars.state = 0x08
            case S_TXDATA:
                activity_ti5(referenceTime);
                break; //ieee154e_vars.dataToSend->l4_destination_port = 61617
            case S_RXACK:
...
port_INLINE void activity_ti1ORri1(void) {
    ...
    */
    ...
    //ieee154e_vars.dataToSend->l4_length == 56
    //ieee154e_vars.dataToSend->l4_protocol == 0x11
//ieee154e_vars.dataToSend->creator == 0x24
//ieee154e_vars.dataToSend->l4_destination_port == 61617
...
//                     (errorparameter_t)ieee154e_vars.slotOffset);
                  ...
                  //  if (ieee154e_vars.dataToSend->creator == COMPONENT_UINJECT && ieee154e_vars.dataToSend->l4_destination_port == WKP_UDP_INJECT) {debugpins_frame_set();debugpins_frame_clr();} //added by mm
  //                      if (ieee154e_vars.dataToSend->creator == 0x24 && ieee154e_vars.dataToSend->l4_length == 56) {debugpins_frame_toggle();debugpins_frame_toggle();} //added by mm [UINJECT_SEND]
...
//if (ieee154e_vars.dataToSend->l4_length == 56) {debugpins_frame_toggle();debugpins_frame_toggle();} //added by mm [UINJECT_SEND]
...
port_INLINE void activity_ti5(PORT_TIMER_WIDTH capturedTime) {
    ...
    if (listenForAck==TRUE) {
      ...
      //  if(ieee154e_vars.dataToSend->creator == 0x24) debugpins_frame_toggle();
        // indicate to upper later the packet was sent successfully
        notif_sendDone(ieee154e_vars.dataToSend,E_SUCCESS); //ieee154e_vars.dataToSend->creator = 0x24
...
// {debugpins_frame_toggle();debugpins_frame_toggle();} //added by mm [UINJECT_RECEIVE]
 
// if (ieee154e_vars.dataReceived->l4_destination_port == 61617) {debugpins_frame_toggle();debugpins_frame_toggle();} //added by mm [UINJECT_RECEIVE]
        ...
        if (ieee802514_header.ackRequested==1 && ieee154e_vars.isAckEnabled == TRUE) {
            ...
            endSlot();
                      ...
                      // {debugpins_frame_toggle();debugpins_frame_toggle();} //added by mm [UINJECT_RECEIVE], until here the reception is done correctly
        // everything went well, return here not to execute the error code below
        return;
...
//if (ieee154e_vars.dataReceived != NULL){debugpins_frame_toggle();debugpins_frame_toggle();} //added by mm [UINJECT_RECEIVE]
                ...
                if (oldFrameLength==0){
                    ...
                    for (i=0;i<numlinks;i++){
                        ...
                        schedule_addActiveSlot(
                            ...
                            TRUE  //Since it is TXRX and shared set isHardCell to true (don't understand what this slot is for!!)
                        );
                    }
                }
