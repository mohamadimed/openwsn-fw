...
#define DIO_PORTION 30//4 //10 for single phy, 30 for g6tisch 4 for local testing
#define DAO_PORTION 60//15 //original 60, for local testing 15
      ...
      if (neighbors_getNeighborKey(&NeighborAddress, ADDR_64B, &NeighborRadioSetting, i)) { // this neighbor entry is in use
         if (packetfunctions_sameAddress(&(msg->l2_nextORpreviousHop),&NeighborAddress)) { // matching address 
            ...
            if (
              ...
              (icmpv6rpl_vars.incomingDio->rank - neighborRank) > ((3*radioDefaultLinkCostMap[NeighborRadioSetting]-2)*MINHOPRANKINCREASE * MAX_RANK_WEIGHT) // 6 is Max Rank 
            ) {
               // the new DAGrank looks suspiciously high, only increment a bit
               
               neighbors_setNeighborRank(i,neighborRank + ((3*radioDefaultLinkCostMap[NeighborRadioSetting]-2)*2*MINHOPRANKINCREASE * MAX_RANK_WEIGHT));
            ...
            if  (max_radios_per_mote ==0){
              break; // there are no more expected matching entries, no need to loop further
            }
         }
      }
   }
...
void sendDIO(void) {
    ...
    *(msg->payload+3) = icmpv6rpl_vars.dio.rank        & 0xFF;
    ...
    if (icmpv6_send(msg)==E_SUCCESS) {
        ...
        if (icmpv6rpl_vars.dioCounter >= 1){
