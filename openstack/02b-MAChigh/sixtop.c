...
owerror_t sixtop_request(
    ...
    sixtop_vars.isHardCell = isHardCell; //set local vars to isHardCell Value
    
    pkt->l2_isHardCell = isHardCell;
    pkt->l2_cellRadioSetting = CELLRADIOSETTING_FALLBACK;
    ...
    //append if isHardCell or not
        if (code == IANA_6TOP_CMD_ADD){
        // append 6p celloptions
        packetfunctions_reserveHeaderSize(pkt,sizeof(uint8_t));
        *((uint8_t*)(pkt->payload)) = isHardCell;
        len+=1;
    }
    // append 6p Seqnum and schedule Generation
    packetfunctions_reserveHeaderSize(pkt,sizeof(uint8_t));
    
    sequenceNumber              = neighbors_getSequenceNumber(neighbor,pkt->l2_cellRadioSetting);
...
owerror_t sixtop_send_internal(
    ...
    if (
        ...
        msg->l2_retriesLeft = 1;//TXRETRIES + 1;
    }
    ...
    if (
        ...
        moteAddr = &(msg->l2_nextORpreviousHop);
        moteId = (((uint16_t)(moteAddr->addr_64b[6]))<<8) + (uint16_t)(moteAddr->addr_64b[7]);
        // what if this fails?
        schedule_addActiveSlot(
            msf_hashFunction_getSlotoffset(moteId,superSlotLengthMap [CELLRADIOSETTING_FALLBACK]),    // slot offset
            ...
            FALSE                                                             //Hard Cell? no, it's set temporarly to send paket downstream because we dont have negoociated cells
        );
    }
    return E_SUCCESS;
}
...
//=========================== Compose eb payload ==============================
    ...
    if (ebIEsBytestream[EB_SLOTFRAME_NUMLINK_OFFSET]>1){
      uint8_t runningOffset = 0;
        for (i=0;i<ebIEsBytestream[EB_SLOTFRAME_NUMLINK_OFFSET];i++){
            packetfunctions_reserveHeaderSize(eb,5);
            eb->payload[0]   = runningOffset;    // slot offset
            eb->payload[1]   = 0x00;
            eb->payload[2]   = 0x00; // channel offset
            eb->payload[3]   = 0x00;
            eb->payload[4]   = cellLinkOptions[i]; // link options
            runningOffset    += superSlotLengthMap[i];
    ...
    if (msg->l2_sixtop_messageType == SIXTOP_CELL_REQUEST){
        if(error == E_FAIL) {
            ...
            case SIX_STATE_WAIT_ADDREQUEST_SENDDONE:
                // mark the neighbor as no resource and update parent selection
                neighbors_setNeighborNoResourceByAddress(&(msg->l2_nextORpreviousHop));
//                openserial_printError(
//                    COMPONENT_SIXTOP,
//                    ERR_6PTIMEOUT,
//                    (errorparameter_t)SIX_STATE_WAIT_ADDREQUEST_SENDDONE,
//                    (errorparameter_t)error
//                );                
                sixtop_vars.six2six_state = SIX_STATE_IDLE;
                break;
        ...
        if(error == E_SUCCESS) {
            ...
            if (msg->l2_sixtop_returnCode == IANA_6TOP_RC_SUCCESS){
                if (msg->l2_sixtop_command == IANA_6TOP_CMD_ADD){
                  // This is where the first problem happens, parent does not add the new cell
                    sixtop_addCells(
                        msg->l2_sixtop_frameID,
                        msg->l2_sixtop_celllist_add,
                        &(msg->l2_nextORpreviousHop),
                        msg->l2_sixtop_cellOptions,
                        msg->l2_isHardCell                  //isHardCell?
...
/*To restore as it was*/
            seqnumber = neighbors_getSequenceNumber(&(pkt->l2_nextORpreviousHop),pkt->l2_cellRadioSetting);
            
   /* openserial_printError(COMPONENT_IEEE802154E,ERR_MAXTXDATAPREPARE_OVERFLOW,
                         (errorparameter_t)seqNum,
                         (errorparameter_t)seqnumber);
                openserial_printError(COMPONENT_IEEE802154E,ERR_MAXTXDATAPREPARE_OVERFLOW,
                         (errorparameter_t)pkt->l2_cellRadioSetting,
                         (errorparameter_t)pkt->l2_cellRadioSetting);*/
            // sequenceNumber check
            if (seqNum != seqnumber && code != IANA_6TOP_CMD_CLEAR){
/*Until Here*/              
            ...
            // commands check
              ...
              if (code == IANA_6TOP_CMD_ADD){
              // extract if isHardCell 
              isHardCell  = *((uint8_t*)(pkt->payload)+ptr);
              ptr         += 1;
              pktLen      -= 1;
            }
            ...
            // add command
            if (code == IANA_6TOP_CMD_ADD){
                if (schedule_getNumberOfSuperSlotFreeEntries(superSlotLengthMap[cellRadioSetting]) < numCells){
                //if (schedule_getNumberOfFreeEntries() < numCells){// look here
                ...
                }/*else{ // if none of these cells are not available, others are. 
                    // none of the slots are available
                    returnCode = IANA_6TOP_RC_BUSY; // look here
                    break;
                }*/
                returnCode = IANA_6TOP_RC_SUCCESS;
                break;
        ...
        // Say if isHardCell as demamned 
        response_pkt->l2_isHardCell = isHardCell;
    ...
    if (type == SIXTOP_CELL_RESPONSE) {
      ...
      // if the code is SUCCESS
        if (code == IANA_6TOP_RC_SUCCESS || code == IANA_6TOP_RC_EOL){
            ...
            //Get if isHardCell from local vars of sioxtop
            isHardCell = sixtop_vars.isHardCell;
            ...
            case SIX_STATE_WAIT_ADDRESPONSE:
                ...
                // this is where the second problem happens. child does not add the new cell
                sixtop_addCells(
        ...
        } else { //Comme le RC == SEQNUM_ERR, alors il affiche un message d'erreur sans continuer le processus d'allocation
            // this ultimately goes to msf_handleRCError
...
bool sixtop_areAvailableCellsToBeScheduled(
    ...
    if(numOfCells == 0 || numOfCells>CELLLIST_MAX_LEN){
        ...
        do {
            if(schedule_isSlotOffsetAvailable(cellList[i].slotoffset, superSlotLengthMap[cellList[i].cellRadioSetting]) == TRUE){
