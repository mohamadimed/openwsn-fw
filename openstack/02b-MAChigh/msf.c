...
void msf_init(void) {
    ...
    moteAddr = idmanager_getMyID(ADDR_64B);
    ...
    schedule_addActiveSlot(
        ...
        TRUE                                                             //Hard Cell? consider Autonoumous RX cell as hard cell (TRUE)                                        
    );
...
void msf_handleRCError(uint8_t code, open_addr_t* address, cellRadioSetting_t cellRadioSetting){
    ...
    if (code==IANA_6TOP_RC_BUSY){ // look here
...
void msf_timer_clear_task(void){
  ...
  uint8_t cellOptions = CELLOPTIONS_MSF | (neighborRadio << 5);
    sixtop_request(
        ...
        FALSE                        //isHardCell? no MSF triggers clear when an error occures with its parent and clears the nogociated cells
    );
}
...
void msf_trigger6pAdd(void){
    ...
    sixtop_request(
        ...
        0,                           // list command offset (not used)
        0,                            // list command maximum celllist (not used)
        FALSE                           //isHardCell? no, MSF triggers adding soft cells
    );
}
...
void msf_trigger6pDelete(void){
    ...
    if (msf_candidateRemoveCellList(celllist_delete,&(neighbor),neighborRadio,NUMCELLS_MSF, cellOptions)==FALSE){
    ...
    sixtop_request(
        ...
        0,                      // list command offset (not used)
        0,                       // list command maximum celllist (not used)
        FALSE                    //isHardCell? no, MSF triggers delete of soft cells
    );
}
...
//        slotoffset = openrandom_get16b()%schedule_getFrameLength();
        slotoffset = msf_hashFunction_getSlotoffset ( openrandom_get16b(), superSlotLengthMap[cellRadioSetting]);
        if(schedule_isSlotOffsetAvailable(slotoffset, superSlotLengthMap[cellRadioSetting])==TRUE){
...
/*modified by mm to delete per neighbor address and radio*/
...
bool msf_candidateRemoveCellList(
    ...
    for(i=0;i<schedule_getFrameLength();i++){
        ...
        if(
            ...
            info.cellRadioSetting == cellRadioSetting
        ){
...
void msf_housekeeping(void){
    ...
    if (schedule_hasNegotiatedTxCellToNonParent(
          ...
          sixtop_request(
              ...
              0,                       // list command offset (not used)
              0,                        // list command maximum celllist (not used)
              FALSE                     //isHardCell? consider it's not a hard cell to realocate (reallocation is handled in MSF_housekeeping -> schedule_getCellsToBeRelocated)
          );
...
// Mina Rady Dec 2020
uint16_t msf_hashFunction_getSlotoffset(uint16_t randomInput, uint8_t superSlotLength){

//    uint16_t moteId;
    uint8_t i;
    uint8_t activeMicroSlots = 0;
    
    //moteId = (((uint16_t)(address->addr_64b[6]))<<8) + (uint16_t)(address->addr_64b[7]);
    
    
    for (i=0;i<SCHEDULE_MINIMAL_6TISCH_ACTIVE_CELLS; i++)
    {
      activeMicroSlots += superSlotLengthMap [i];
    }
    return activeMicroSlots + \
            (randomInput%( (SLOTFRAME_LENGTH-activeMicroSlots-superSlotLength) /superSlotLength ))*superSlotLength;
}
