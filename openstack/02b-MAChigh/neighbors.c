...
uint8_t neighbors_getSequenceNumber(open_addr_t* address, cellRadioSetting_t cellRadioSetting){
    uint8_t i;
    for (i=0;i<MAXNUMNEIGHBORS;i++){
      if (packetfunctions_sameAddress(address, &neighbors_vars.neighbors[i].addr_64b) ){//&&  neighbors_vars.neighbors[i].cellRadioSetting == cellRadioSetting){
            break;
        }
    }
...
void neighbors_updateSequenceNumber(open_addr_t* address, cellRadioSetting_t cellRadioSetting){
    uint8_t i;
    for (i=0;i<MAXNUMNEIGHBORS;i++){
            //DISABLE update per neighbor, per radio to avoid SeqNum conflicts which create errors whille adding new cells
        if (packetfunctions_sameAddress(address, &neighbors_vars.neighbors[i].addr_64b) ){//&& neighbors_vars.neighbors[i].cellRadioSetting == cellRadioSetting){
...
void neighbors_setNeighborNoResourceByAddress(open_addr_t* address){
   uint8_t i;
   uint8_t max_radios_per_mote = MAX_CELLRADIOSETTINGS;
   // loop through neighbor table
   for (i=0;i<MAXNUMNEIGHBORS;i++) {
      if (isThisRowMatchingAddress(address,i)) {
          max_radios_per_mote--;
          neighbors_vars.neighbors[i].f6PNORES = TRUE;
          icmpv6rpl_updateMyDAGrankAndParentSelection();
          if (max_radios_per_mote==0){
            break;
          }
      }
   }
...
uint16_t neighbors_getLinkMetric(uint8_t index) {
    ...
    if (neighbors_vars.neighbors[index].numTxACK==0) {
        if (neighbors_vars.neighbors[index].numTx > radioDefaultLinkCostMap[neighborRadio]){
            ...
            rankIncrease = (3*radioDefaultLinkCostMap[neighborRadio]-2)
              * MINHOPRANKINCREASE 
              * radioMinHopRankIncreaseFactor [neighborRadio];
        }
    } else {
...
void registerNewNeighbor(open_addr_t* address,
    ...
    if (isNeighbor(address,cellRadioSetting)==FALSE) {
       ...
       // printf("ADD %u %u",neighbors_vars.neighbors[0].addr_64b.addr_64b[0],neighbors_vars.neighbors[0].addr_64b.addr_64b[7]);
    }
}
...
bool isThisRowMatchingAddress(open_addr_t* address, 
                       uint8_t rowNumber) {
   switch (address->type) {
      case ADDR_64B:
         return neighbors_vars.neighbors[rowNumber].used &&
