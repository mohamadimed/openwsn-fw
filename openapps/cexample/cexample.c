...
#define CEXAMPLEPERIOD  5000 //10000 //modified by mm init 10000
...
void cexample_task_cb(void) {
    ...
    for (i=0;i<PAYLOADLEN;i++) {
        pkt->payload[i]             = i;                        //When parsing the paclet, the first 11 bytes are to be ignored, data starts after that in app_payload[], or ignor 19 byte if handling payload[]
    }
    ...
    memcpy(&pkt->l3_destinationAdd.addr_128b[0],&ipAddr_ringmaster,16); //see opencoap.h to select the appropriate address (think ringMaster address is the most suitable) than: ipAddr_motesEecs
