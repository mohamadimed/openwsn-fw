...
void cjoin_init(void) {
   ...
   //cjoin_schedule();
    ...
    if (cjoin_getIsJoined() == FALSE) {
        delay = openrandom_get16b()*SLOW_JOIN_INTERVAL;
...
void cjoin_schedule(opentimers_id_t id) {
    uint16_t delay;
...
//{debugpins_frame_toggle();debugpins_frame_toggle();}
    ...
    options[2].type = COAP_OPTION_NUM_OBJECTSECURITY;  //HERE is the problem invalid option number 21 see inital def of cjoin in github COAP_OPTION_NUM_OSCORE
