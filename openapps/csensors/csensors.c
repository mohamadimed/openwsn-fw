...
void csensors_fillpayload(OpenQueueEntry_t* msg,
   ...
   uint16_t              value;                                                                                         //s/c (sensors.c)

   value=csensors_vars.csensors_resource[id].opensensors_resource->callbackRead();
   //next line added by mm to get a readable value of the request ressource, because for eg, in /s/c : result is 36101
   //value=csensors_vars.csensors_resource[id].opensensors_resource->callbackConvert(value); --> must use this adc_sens_convert_temperature for /s/c (adc_sensors.c)
   //it works now for s/c/, but for t and h it gives alwayes 128, maybe because I forced si70x_is_present() [converted should be FLOAT type]--> find a way to send float or convert in python
   
   packetfunctions_reserveHeaderSize(msg,sizeof(uint16_t));

  //I have inverted insertion in payload to treat it correctly on python script
   // add value
   msg->payload[1]                  = (uint8_t)((value & 0xff00)>>8);//(value>>8) & 0x00ff;
   msg->payload[0]                  = (uint8_t)(value & 0x00ff);//value & 0x00ff;
