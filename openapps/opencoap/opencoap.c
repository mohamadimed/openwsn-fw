...
void opencoap_receive(OpenQueueEntry_t* msg) {
   ...
   if (found==TRUE && securityReturnCode==COAP_CODE_EMPTY) {
      ...
      outcome = temp_desc->callbackRx(msg,&coap_header,&coap_incomingOptions[0], coap_outgoingOptions, &coap_outgoingOptionsLen);  //callback to excute RX function in which we handle the coap packet
      if (outcome == E_FAIL) {
