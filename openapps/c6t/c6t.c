   ...
   switch (coap_header->Code) {
      ...
      case COAP_CODE_REQ_PUT:
         ...
         outcome = sixtop_request(
            ...
            TRUE                                //isHardCell? yes, we want to add it on demand
         );
      ...
      case COAP_CODE_REQ_DELETE:
         ...
         outcome = sixtop_request(
            ...
            0,                                  // list command offset (not used)
            0,                                   // list command maximum celllist (not used)
            TRUE                                //isHardCell? yes, we want to delete it on demand (even if here it does not really matters because chek is done on MSF house keeping) we can delete it even if true or false
         );
