...
int mote_main(void) {
    ...
    while (1) {
        ...
        hex2dec( app_vars.rxpk_num,&r,&l,&ll);
        app_vars.uart_txFrame[i++] = app_vars.ll;  // packet number
        app_vars.uart_txFrame[i++] = app_vars.l;  // packet number
        app_vars.uart_txFrame[i++] = app_vars.r;  // packet number
        
        hex2dec(app_vars.rxpk_rssi,&r,&l,&ll);
        app_vars.uart_txFrame[i++] = app_vars.ll;
        app_vars.uart_txFrame[i++] = app_vars.l;
        app_vars.uart_txFrame[i++] = app_vars.r;
...
void hex2dec(uint8_t i,uint8_t* r,uint8_t* l,uint8_t* ll)
{
*ll = i/100;
 i = i-*ll*100;
// printf("%d\n",i);
