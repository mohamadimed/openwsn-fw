...
void radio_setConfig_at86rf215(radioSetting_t radioSetting){
    uint16_t _register;
    radio_api.selectedRadioSetting = radioSetting;
    at86rf215_spiStrobe(CMD_RF_TRXOFF, ATMEL_FREQUENCY_TYPE);
    while(at86rf215_status(ATMEL_FREQUENCY_TYPE) != RF_STATE_TRXOFF);
...
void radio_setFrequency_at86rf215(uint16_t channel, radio_freq_t tx_or_rx) {
    ...
    if(ATMEL_FREQUENCY_TYPE==0){
        if (radio_api.selectedRadioSetting >= RADIOSETTING_OFDM_OPTION_1_MCS0 && 
            radio_api.selectedRadioSetting <= RADIOSETTING_OFDM_OPTION_1_MCS3 ){
            frequency_0 = (DEFAULT_CENTER_FREQUENCY_0_FSK_OPTION_1/25);
            channel_spacing = 0x30; // section 7.2.1 pp 170 atmel data sheet
            channel = channel%5;
        }else{    
            frequency_0 = (DEFAULT_CENTER_FREQUENCY_0_FSK_OPTION_1/25);
            channel_spacing = (uint8_t)(DEFAULT_CHANNEL_SPACING_FSK_OPTION_1/25);
        }
        at86rf215_spiWriteReg(RG_RF09_CS, channel_spacing);
